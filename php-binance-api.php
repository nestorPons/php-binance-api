...
class API
{
     ...
     */
    public function symbolInfo(string $symbol) : array
    {
        $return = []; 
        foreach( $this->exchangeInfo()['symbols'] as $key => $value)
        {
            if ($symbol == $key) $return = $value;
        }
        return $return;
    }
    /**
     ...
     * @throws \Exception
     */
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)  : ?array
    {
